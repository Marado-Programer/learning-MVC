...
class NewsCreateModel extends MainModel
{
    ...
    public function editNews(int $id)
    {
        if (!UsersManager::getTools()->permissionManager->checkPermissions(
            $this->userAdmniPermissions($this->controller->user, $this->controller->association),
            PermissionsManager::AP_EDIT_NEWS,
        ))
            return;
        ...
        */
        ...
        $this->db->update(
            ...
            ['id' => $id],
            [
                'requestedArticle' => $news['article'],
                'requestedImage' => $news['image']['name'],
                'requestedTitle' => $news['title']
            ]
        );
        ...
        $_SESSION['news-created'] = 'A edit was submited.';
